...
grep -q 'firmware_args=(-drive.*readonly=on.*-drive' "$root/test/qemu/test-iso.sh"
...
! grep -q 'network-online.target' "$root/release/elderbrain-stack.service"
! grep -q 'network-online.target' "$root/provisioning/systemd/elderbrain-graphics.service"
grep -q -- 'systemd-networkd-wait-online --any --dns -o routable --timeout=30' "$root/provisioning/systemd/wait-online-timeout.conf"
grep -q '^SuccessExitStatus=1$' "$root/provisioning/systemd/wait-online-timeout.conf"
