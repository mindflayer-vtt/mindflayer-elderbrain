...
After selection, Subiquity performs the base installation from the ISO without cloning this repository. Provisioning requires working Internet and DNS to install packages and pull the pinned Traefik and Mindflayer containers; Foundry downloads later through its own container. A failed pull is visible in `systemctl status elderbrain-stack` and `journalctl -u elderbrain-stack`, and systemd retries rather than presenting a partially healthy stack. Browse to `http://<appliance-ip>/elderbrain/` if local DNS is absent. The first display also opens this URL. DNS records for the configured hostnames must be created externally.
