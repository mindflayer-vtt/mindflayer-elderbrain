...
commit=$(git -C "$ROOT" rev-parse HEAD); dirty=''; git -C "$ROOT" diff --quiet && git -C "$ROOT" diff --cached --quiet || dirty=-dirty
printf '%s%s\n' "$commit" "$dirty" > "$WORK/tree/elderbrain/VERSION"
cp "$ROOT/iso/autoinstall/user-data.in" "$WORK/tree/user-data"
printf 'instance-id: elderbrain-%s\nlocal-hostname: elderbrain\n' "${commit:0:12}" > "$WORK/tree/meta-data"
if [[ -n $key ]]; then
...
xorriso -as mkisofs -r -V 'MINDFLAYER' -o "$OUT/mindflayer-elderbrain-${commit:0:12}.iso" \
...
echo "$OUT/mindflayer-elderbrain-${commit:0:12}.iso"
